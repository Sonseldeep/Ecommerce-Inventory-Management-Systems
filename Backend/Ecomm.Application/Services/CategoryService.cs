...
public class CategoryService : ICategoryService
{
    ...
    public CategoryService(
        ...
        IValidator<UpdateCategoryRequestDto> updateValidator)
    {
    ...
    public async Task<PagedResult<CategoryResponseDto>> SearchAsync(CategoryQueryParamsDto query, CancellationToken ct = default)
    {
        ...
        var categoriesWithCounts = new List<CategoryResponseDto>();
        foreach (var category in items)
        {
            var productCount = await _products.Query()
                .CountAsync(p => p.CategoryId == category.Id && !p.IsDeleted, ct);

            categoriesWithCounts.Add(category.ToDto(productCount));
        }
