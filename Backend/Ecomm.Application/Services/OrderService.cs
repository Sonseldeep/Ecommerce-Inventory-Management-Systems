...
//             if (product.QuantityInStock < item.Quantity) throw new BadRequestException($"Insufficient stock for '{product.Name}'.");
//
...
//         {
//             var product = await _products.GetByIdAsync(line.ProductId, ct) ?? throw new NotFoundException("Product not found.");
...
//
//         _logger.LogInformation("Order created. OrderNumber: {OrderNumber}, UserId: {UserId}", created.OrderNumber, userId);
//
