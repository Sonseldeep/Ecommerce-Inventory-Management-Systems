    ...
    <div className="p-6 grid lg:grid-cols-2 gap-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        <img 
          ...
          alt={product.name}
          className="w-full h-[420px] object-cover rounded-xl transition-all duration-300" 
        />
        <div className="flex gap-2 mt-3 overflow-auto pb-2">
          {(product.images || []).map((im) => (
            <img
              ...
              onClick={() => setImg(im.imageUrl)}
              className={`w-16 h-16 rounded border cursor-pointer object-cover ${
                img === im.imageUrl ? "border-black border-2" : "border-gray-200"
              }`}
            />
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl shadow p-6 flex flex-col justify-between">
        <div>
          <h1 className="text-3xl font-bold">{product.name}</h1>
          <p className="text-gray-500 mt-1 uppercase tracking-wide text-sm">{product.categoryName}</p>
          <p className="mt-4 text-gray-700 leading-7">{product.description}</p>
          ...
          {/* Stock Display */}
          <p className={`text-sm mt-2 font-medium ${product.quantityInStock <= 0 ? "text-red-500" : "text-gray-500"}`}>
