...
//     await load();
//   } catch (e) {
//     console.log("update cart error", e?.response?.data || e.message);
...
//   } catch (e) {
//     console.log("remove cart error", e?.response?.data || e.message);
...
//               <button className="border px-2" onClick={() => updateQty(i.id, i.quantity - 1)}>-</button>
//               <span>{i.quantity}</span>
//               <button className="border px-2" onClick={() => updateQty(i.id, i.quantity + 1)}>+</button>
//               <button className="text-red-600 ml-3" onClick={() => removeItem(i.id)}>Remove</button>
//             </div>
//           </div>
//         ))}
//       </div>

//       <div className="mt-6 bg-white rounded-xl shadow p-4 flex justify-between items-center">
//         <p className="font-bold">Total: ₹ {total.toFixed(2)}</p>
//         <button onClick={() => navigate("/checkout")} className="bg-black text-white px-4 py-2 rounded">
//           Proceed to Checkout
//         </button>
//       </div>
//     </div>
...
//   const total = useMemo(
//     () => (cart?.items || []).reduce((s, i) => s + Number(i.unitPrice || 0) * Number(i.quantity || 0), 0),
//     [cart]
//   );
...
//       {(cart.items || []).map((i) => (
//         <div key={i.id} className="bg-white rounded-2xl shadow p-4 flex justify-between items-center">
//           <div>
//             <p className="font-semibold">{i.productName}</p>
//             <p className="text-sm text-gray-500">₹ {i.unitPrice}</p>
//           </div>
//           <div className="flex items-center gap-2">
//             <button className="w-8 h-8 border rounded" onClick={() => updateQty(i.id, i.quantity - 1)}>-</button>
//             <span>{i.quantity}</span>
//             <button className="w-8 h-8 border rounded" onClick={() => updateQty(i.id, i.quantity + 1)}>+</button>
//             <button className="ml-3 text-red-600" onClick={() => removeItem(i.id)}>Remove</button>
//           </div>
//         </div>
//       ))}
