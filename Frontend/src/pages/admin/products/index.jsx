...
export default function AdminProductsPage() {
  ...
  // ── Hook ─────────────────────────────────────────────────────────
  // `gridRef`       → forwarded to ProductsGrid so the hook can call .refresh()
  // `loadCategories`→ replaces the old `loadProducts` — we only need categories
  //                   because the grid's CustomStore fetches products itself
  const {
    gridRef,
    categories,
    loadingData,
  ...
  // The grid will load page 1 of products automatically via its CustomStore.
  useEffect(() => {
    loadCategories();
  }, [loadCategories]);
    ...
    >
      ...
      {!loadingData && (
        <>
          ...
          {activeTab === "analytics" && (
            // Analytics still uses the local products array.
            // If you want analytics to also reflect all 27 products (not just
            // the current page), fetch all products separately here or pass
            // a dedicated "all products" state from the hook.
            <ProductsAnalytics
              products={[]}         // replace with a full-fetch if needed
              categories={categories}
            />
          )}
        </>
